  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" style={{ fontFamily: "var(--font-display)", color: CREAM }}>Clients</h1>
      ...
      {clients.length === 0 ? (
        <div className="rounded-2xl p-12 text-center" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
          ...
          <p className="font-medium mb-1" style={{ color: CREAM }}>No clients yet</p>
              ...
              <div key={client.id} className="flex items-center justify-between px-6 py-4 transition-colors" style={{ borderBottom: `1px solid ${BORDER}` }}>
                <div className="flex items-center gap-4">
                  <div
                    ...
                    style={{ background: BORDER, color: CREAM }}
                  >
                    {client.name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <p className="font-medium" style={{ color: CREAM }}>{client.name}</p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="rounded-2xl w-full max-w-md" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
            ...
            <form onSubmit={editData ? handleUpdate : handleCreate} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: CREAM }}>Name *</label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: CREAM }}>Email</label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: CREAM }}>Address</label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: CREAM }}>VAT ID</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => { setShowForm(false); setEditingClient(null); }} className="flex-1 py-2.5 rounded-xl text-sm font-medium transition-colors" style={{ border: `1px solid ${BORDER}`, color: MUTED }}>
                  Cancel
                </button>
